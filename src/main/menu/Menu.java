...
public class Menu {
    ...
    public void getItemPositions() {
        ...
        for (String item : this.items) {
            ...
            cx = (Settings.BASE_SCREEN_WIDTH / 2) - ((item.length() * Settings.BASE_TILE_SIZE) / 2);

            this.itemPositions[count][0] = cx;
            this.itemPositions[count][1] = cy;

            cy += Settings.BASE_TILE_SIZE * 2;
    ...
    public void setImage() {
        ...
        BufferedImage baseDisplay = new BufferedImage(Settings.BASE_SCREEN_WIDTH, Settings.BASE_SCREEN_HEIGHT, BufferedImage.TYPE_INT_RGB);
        Graphics g = baseDisplay.getGraphics();

        BufferedImage titleTextImage = this.gp.spritesheet.getText(this.title);

        int x = (Settings.BASE_SCREEN_WIDTH / 2) - (titleTextImage.getWidth() / 2);
        int y = 5 * Settings.BASE_TILE_SIZE;
        
        g.drawImage(titleTextImage, x, y, titleTextImage.getWidth(), titleTextImage.getHeight(), null);

        y += 5 * Settings.BASE_TILE_SIZE;
        ...
        for (int i = 0 ; i < this.itemCount ; i++) {
            ...
            if (!this.items[i].equals("ph")) {
                ...
                g.drawImage(textImage, this.itemPositions[i][0], this.itemPositions[i][1] + y, textImage.getWidth(), Settings.BASE_TILE_SIZE, null);

            }

            y += Settings.BASE_TILE_SIZE;
    ...
    public BufferedImage getImage() {
        ...
        int x = (Settings.BASE_SCREEN_WIDTH / 2) - (titleTextImage.getWidth() / 2);
        int y = 6 * Settings.BASE_TILE_SIZE;
        
        g.drawImage(titleTextImage, x, y, titleTextImage.getWidth(), titleTextImage.getHeight(), null);

        y += 2 * Settings.BASE_TILE_SIZE;
